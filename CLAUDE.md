...
| **witslog-mcp** | MCP tool registry + JSON-RPC transport | `registry.rs` (13 built-in tools incl. read-only `mttr`/`get_event`, + opt-in `search_all` and gated `witslog_delete`), `transport.rs` (stdio JSON-RPC), `tools.rs` (schema) — wired into CLI as `witslog serve-mcp` |
| **witslog-cli** | CLI subcommands (init/log/get/query/stats/export/import/vacuum/prune/config/archive/backup/list-dbs/migrate/resolve/delete/doctor/category/serve-mcp/uninstall) | `main.rs` (clap Commands; global `--json`/`--color`, `query --unresolved`, `stats --mttr`, `resolve --force`, `delete --dry-run` via `preview_delete`) |
...
- **Any doc bundled/shipped standalone by a package manager must use absolute GitHub URLs, never relative paths — this is a standing rule, not a one-file exception.** Applies to every file a package manager ships outside the full repo tree: `bindings/node/{CHANGELOG,README}.md` (npm — `README.md` is *always* bundled by npm regardless of `package.json`'s `files` array, and `CHANGELOG.md` is bundled because it's explicitly listed there), `bindings/python/{CHANGELOG,README}.md` (PyPI sdist/wheel — see `MANIFEST.in`), `bindings/php/{CHANGELOG,README}.md` (Packagist ships the whole `bindings/php/` tree today, but don't assume that stays true forever). A reader hits these on the npm/PyPI/Packagist package page or inside `node_modules/`/`site-packages/`/`vendor/` with **no surrounding repo tree** — any relative link (`../CONTRACT.md`, `README.md`, `../../CHANGELOG.md`, `../browser`) 404s or resolves to nothing. Always link as `https://github.com/all-wits/witslog/blob/main/<path>` (a link to another file in the *same* directory needs the full path too — `README.md` is wrong, `https://github.com/all-wits/witslog/blob/main/bindings/python/README.md` is right). Past entries have drifted back to relative paths (e.g. `bindings/python/CHANGELOG.md`'s and `bindings/php/CHANGELOG.md`'s self-referencing `[README.md](README.md)`, fixed in the same session this rule was generalized) — check every link, in every bundled doc, every time one of these files is edited. Root `CHANGELOG.md`, root `README.md`, and other in-repo-only docs (`PLAN.md`, `PHASES.md`, `CLAUDE.md` itself) are exempt and can keep normal relative links — they are never shipped standalone, always read inside the repo tree (GitHub web view or a local clone).
...
- ✅ **P4**: CLI ops — query/stats/export/import/vacuum/prune/migrate/config/archive/backup/list-dbs/category all shipped; global `--json` and `--color <auto|always|never>` flags shipped (`style.rs`).
- ✅ **P5**: MCP server — witslog-mcp crate (13 tools incl. `get_event`, JSON-RPC stdio transport, schema validation, statement timeout), wired into CLI as `witslog serve-mcp [--stdio] [--attach] [--allow-write]`, conformance test (`p5_integration.rs`) green. `initialize` handles the standard MCP handshake (v0.1.5, previously `-32601` before `tools/list`) and returns worked-example `instructions`; every tool `description` carries a worked `Example: {...}` call + disambiguation against overlapping tools — added after real-world use showed a connected server wasn't enough for a lightweight model (Claude Haiku 4.5) to pick the right tool. See Gotchas.
...
- ✅ **P8**: Packaging + install. Version-compat guard (`witslog-store::CURRENT_SCHEMA_VERSION`, refuses newer-than-binary schema), `serve-mcp --print-mcp-config`, `uninstall [--purge]`, migrate `.bak` restore-on-failure, install scripts (`install/install.sh`/`.ps1`), release workflow with a `smoke_test` job (`.github/workflows/release.yml`: builds per OS, runs the real init/log/query/serve-mcp/doctor/uninstall happy path, gates `publish`), Homebrew/Scoop manifest templates, `docs/install.md`. winget/`.deb`/`.rpm` intentionally out of scope — `cargo install`/npm/pip/composer already cover cross-platform distribution pre-1.0. Workflow confirmed green via `workflow_dispatch` on real Linux/macOS/Windows runners (build matrix + smoke_test). Real `v*.*.*` tags cut and pushed through `v0.1.7` — root crate/CLI version and git tag in sync at `0.1.7`. `@all-wits/witslog` npm package versions independently (`bindings/node/CHANGELOG.md`); `package.json` at `0.6.4` — publishing still requires a manual `release-node-sdk.yml` (`workflow_dispatch`, `publish:true`) run, it is not tag-triggered.
- 🟡 **P9**: Extensibility + security. `witslog-plugin` crate (6 traits + panic-isolated registry). Audit hash chain (`migrate_0006_audit_chain`, `witslog-store::audit`, `witslog doctor --verify-audit`). File-perm hardening (0600 DB, Unix). Metadata-field encryption (`witslog-core::crypto::FieldCipher`, AES-256-GCM on `metadata` only) is now **wired end-to-end**, not just a library primitive: opt-in via `[crypto] key_env = "..."` in `config.toml` (`witslog_config::CryptoSection`) / `{"crypto":{"key_env":"..."}}` in `witslog_configure`, fail-closed on write if the key can't be resolved, `"<encrypted>"` placeholder on read without the key — see the Gotcha below. Full SQLCipher whole-DB encryption deliberately deferred (conflicts with FTS5/generated columns; see PHASES.md §P9). Config-driven redaction was already done in P1.
...
1. **P9**: revisit dynamic plugin loading and full DB-at-rest encryption if a real need shows up — see PHASES.md §P9 for why both were scoped down.
2. **P10**: Python/PHP browser-ingest adapters if the Node one proves out (currently a documented recipe only, `bindings/CONTRACT.md`); `ingest_source` provenance in the payload contract would need an ABI-version bump.
3. **P6**: wire `witslog_bootstrap_project` (see Gotchas) into Python/PHP as a convenience wrapper, matching Node's `init({ createProject: true })` — currently Node-only; Python/PHP still require the separately-installed CLI's `witslog init`.
4. **P6**: bundle the CLI binary into the Python/PHP SDKs too (mirroring the Node SDK's `bin/witslog.js` + `_bin/<platform>/` shipped this session) — currently Node-only; Python/PHP `query`/`stats`/`export`/`serve-mcp`/`doctor` still require a separately-installed CLI (no FFI surface for those, by design).
5. **P6**: Python/PHP equivalents of the Node SDK's framework adapters (`frameworks/react-query.js`, `frameworks/axios.js`, `frameworks/hocuspocus.js`) if a real need shows up — currently Node-only, per the "Node SDK framework-adapter contract" in `bindings/CONTRACT.md`.
...
| `Config` | witslog-config/lib.rs | layered config struct (fields: enrich/redact/buffer/taxonomy/notify) |
...
- **MCP tool descriptions must carry a worked `Example: {...}` clause, not just a type-only schema**: a connected, spec-compliant server (`initialize` succeeding, `tools/list` returning valid schemas) is not sufficient for a lightweight/under-informed model to use the tools correctly — confirmed live: Claude Haiku 4.5 failed to retrieve an error list, and even Sonnet 5 (low effort) didn't know the intended workflow, because every `tools.rs::builtin_tools` description was a bare one-liner (`"Most recent failures."`) with no example call and no disambiguation between overlapping tools (`search_errors` requires a non-empty `query` and silently fails for "list the errors" intent; `latest_errors` is the actual right tool and needs none). Fixed via two protocol-native mechanisms — deliberately not a Claude-Code-specific `SKILL.md`, since witslog's MCP server is consumed by Claude Desktop and other MCP clients with no project/skill context: (1) `initialize`'s response now returns a top-level `instructions` string (MCP spec 2024-11-05, previously unused — `server.rs::INITIALIZE_INSTRUCTIONS`), a workflow map with one `tool_name({field: value})` example per step; (2) every tool `description` ends with a worked `Example: {...}` call plus a "use this when / not this when" clause against its nearest overlapping tool, and `severity_min`/`from`/`to`/`query` schema properties gained `enum`/`format`/`examples`. Any new tool must follow the same pattern — regression-locked by `crates/witslog-mcp/src/server.rs::tests::every_tool_description_has_a_worked_example`.
...
- **`FieldCipher` only covers `metadata`, and it's now wired into both write paths and both read paths**: `witslog-core::crypto::FieldCipher::encrypt_metadata` is the only wired encryption hook. `message`/`context`/`stack_norm` etc. stay plaintext because FTS5 and the `GENERATED ALWAYS AS (json_extract(context, ...))` columns need to read them — encrypting those would silently break search and the hot query axes. Opt in via `[crypto] key_env = "WITSLOG_ENCRYPTION_KEY"` in `.witslog/config.toml` (or `{"crypto":{"key_env":"..."}}` for `witslog_configure`/FFI SDK writes) — the named env var must hold a 64-char hex key; the key itself is never in `config.toml`. Two write pipelines needed the change because SDK writes (Python/Node/PHP) never touch `witslog-runtime` — `witslog_runtime::apply_pipeline`/`build_and_write` and `witslog-ffi`'s own `RuntimeConfig`/`witslog_log` pipeline both call `.encrypt_metadata()` after `.redact()`. **Fail-closed on write**: `key_env` set but the var unset/invalid → the write is refused (`witslog_log` returns `-1`, `build_and_write` returns `Err`, ambient `capture` drops the event the same way it already drops on a store-open failure) — metadata is never silently persisted in plaintext. **Placeholder on read**: CLI `get` and MCP `get_event`/`explain_error` call `witslog_core::crypto::decrypt_metadata_for_display(metadata, cipher)` — envelope + no/wrong key → the literal string `"<encrypted>"`, never raw ciphertext, never a failed call; every other field (message/exception/stacktrace/category/context/fingerprint/trace chain) stays fully visible regardless, so an MCP-connected agent without the key can still triage — it only loses what was deliberately put in `metadata`. **Field discipline**: debug signal (request ids, feature flags, non-sensitive state) belongs in `context`/`tags` (always plaintext); PII/secrets belong in `metadata` (the only encrypted field) — mixing the two defeats either the AI's ability to triage or the encryption's purpose. **Key rotation is Level 0 (v1)**: single active key, no rotation code — rotate by letting old rows age out via `[retention]` or `export`→rotate→`import`; a future key-ring (`"kid"` alongside `__witslog_enc`) would be additive/non-breaking if ever needed. `crypto.key_env` in the `witslog_configure` JSON is additive — no `WITSLOG_ABI_VERSION` bump.
...
- **`force` + `resolved_before` interact non-obviously — SQL `NULL <= x` is not false, it's unknown**: `matching_delete_ids` (`witslog-store/src/writer.rs`, shared by `delete_resolved` and `preview_delete` — the CLI's `--dry-run` and the real delete run the exact same query, on purpose, so a preview can never drift from what actually gets deleted) builds `resolved_before` as `(resolved_at IS NULL OR resolved_at <= ?)` only when `force:true`; the bare `resolved_at <= ?` form is used otherwise. Forgetting the `force`-gated widening was the original bug (fixed in `v0.1.7`): under `force`, unresolved rows must stay eligible or `force`'s "delete regardless of resolution state" contract silently breaks for every unresolved row. Any new delete-filter clause must go through `matching_delete_ids`, never a second copy of the query.
...
- **Two Node WebSocket-close adapters exist on purpose, pick by target**: `bindings/browser/witslog-websocket.js::witslogWebSocketWatch` (browser-only, vendored — not an npm subpath) hooks a raw `CloseEvent` and treats any `code` not `1000`/`1001` as abnormal, full stop. `bindings/node/frameworks/hocuspocus.js::attachWitslogHocuspocus` (npm subpath, Node-side) is purpose-built for `HocuspocusProvider`/any `EventEmitter`-shaped target and additionally checks `wasClean` — a clean disconnect can still surface as close code 1005 ("No Status Rcvd", synthesized locally by the browser whenever the server closes without an explicit status, e.g. on routine `provider.destroy()`/tab-nav/HMR reload), and the vendored watcher's code-only check would misreport that as an error. `attachWitslogHocuspocus` also captures `authenticationFailed` (`error_code: COLLAB_AUTH_FAILED`), which the vendored watcher has no hook for at all. Both follow the "Node SDK framework-adapter contract" in `bindings/CONTRACT.md` (duck-typed, `resolveEmit`/`resolveFlush`, `detach()`, eager flush for rare/urgent events).
- **`Redactor::redact`/`EventBuilder::redact` never touch `tags`**: only message/exception/stacktrace/context/metadata are passed through the redaction rules (`witslog-core/src/event.rs::EventBuilder::redact`). Tags are caller-set/classifier-suggested labels, not expected to carry secrets — but if a caller ever puts free-form or user-controlled text into a tag, it persists un-redacted **and** un-encrypted (tags are outside `FieldCipher`'s scope too — see the P9 gotcha above). Don't put anything sensitive in a tag.
- **The browser-ingest rate limiter is in-process, not distributed**: `checkIngestGuardrails`' bucket store (`bindings/node/lib/ingest-core.js`, a plain `Map`) is per handler-instance. This endpoint is local-dev-only by design (see the gotcha above); if it's ever fronted by a load balancer with multiple instances, the effective rate limit becomes `max × instance_count` since each instance tracks its own buckets. Documented in `bindings/CONTRACT.md`'s ingest recipe — don't run this at scale without a shared rate-limit store.

---

Last updated: 2026-07-24 (metadata encryption wired end-to-end, FR-P9-004: `[crypto] key_env` in
`config.toml` / `{"crypto":{"key_env":...}}` in `witslog_configure`, fail-closed write, `"<encrypted>"`
placeholder read, both write pipelines (`witslog-runtime` + `witslog-ffi`) and both read surfaces
(CLI `get`, MCP `get_event`/`explain_error`) — see `witslog-core::crypto::decrypt_metadata_for_display`
and the CHANGELOG `[Unreleased]` entry for the full design. Plus two doc-only fixes: `tags` bypass
redaction/encryption noted, browser-ingest non-distributed rate-limit caveat added to CONTRACT.md.
Ships in a future minor, not `v0.1.7`.)

Previously: 2026-07-23 (release prep for tag `v0.1.7` / npm `@all-wits/witslog@0.6.4`: two `delete`
bugfixes — `force`+`resolved_before`'s `NULL <=` trap and a real `--dry-run` preview via
`preview_delete`/`matching_delete_ids` — plus the new Node-side Hocuspocus/Yjs adapter
(`frameworks/hocuspocus.js::attachWitslogHocuspocus`), documented across CHANGELOG/CONTRACT/READMEs.
Global CLI `--json`/`--color` flags shipped (P4 complete). Root crate/CLI and git tag now at
`0.1.7`; `@all-wits/witslog` at `0.6.4` in `package.json`. Node SDK bundles the real `witslog` CLI
binary per platform — `bin/witslog.js` + `lib/cli-locator.js`, closing the `query`/`stats`/etc.
access gap that `createProject:true` didn't cover. P10 added: MTTR/resolution tracking, notifiers,
browser-side error capture — see PHASES.md §P10). Reflect code reality, not aspirational state.
