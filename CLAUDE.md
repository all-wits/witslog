...
- **`bindings/node/CHANGELOG.md` links must be absolute GitHub URLs, never relative paths.** npm ships this one file standalone in the published tarball (`package.json`'s `files` array) — a reader sees it on the npm package page or in `node_modules/@all-wits/witslog/CHANGELOG.md`, with no surrounding repo tree, so any relative link (`../CONTRACT.md`, `README.md`, `../../CHANGELOG.md`, `../browser`) 404s or resolves to nothing. Always link as `https://github.com/all-wits/witslog/blob/main/<path>` (already the convention for some links in that file — e.g. the `bindings/CONTRACT.md` link in the `0.4.1` entry — but past entries drifted back to relative paths; watch for this every time the file is edited). This applies **only** to `bindings/node/CHANGELOG.md` (and, if a future Python/PHP changelog gets committed to writing similar cross-links, those too) — root `CHANGELOG.md` and other in-repo docs can use normal relative links since they're always read inside the repo tree.
