...
Last updated: 2026-07-23 (MCP `initialize` handshake fixed and MCP tools made self-teaching — worked-example `instructions` + per-tool `Example: {...}` clauses, after real-world use showed a lightweight model couldn't otherwise find the right tool; root crate/CLI tagged `v0.1.5`, `@all-wits/witslog` bumped to `0.6.2` in `package.json` but not yet published to npm. Node SDK bundles the real `witslog` CLI binary per platform — `bin/witslog.js` + `lib/cli-locator.js`, closing the `query`/`stats`/etc. access gap that `createProject:true` didn't cover. P10 added: MTTR/resolution tracking, notifiers, browser-side error capture — see PHASES.md §P10). Reflect code reality, not aspirational state.
